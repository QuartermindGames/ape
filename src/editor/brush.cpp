...
================
*/
static void BeginTexturingFace( Brush *b, face_t *f, qtexture_t *q ) {
	vec3_t	pvecs[ 2 ];
	int		sv, tv;
	float	ang, sinv, cosv;
	float	ns, nt;
...
=============
*/
Brush::Brush( vec3_t mins, vec3_t maxs, texdef_t *texdef ) : Brush() {
	int		i, j;
	vec3_t	pts[ 4 ][ 2 ];
	face_t *f;
...
=================
*/
winding_t *Brush::MakeFaceWinding( face_t *face ) {
	winding_t *w;
	face_t *clip;
	plane_t			plane;
	qboolean		past;
...
void Brush::SnapPlanepts() {
	for( face_t *f = brush_faces; f; f = f->next )
		for( int i = 0; i < 3; i++ )
			for( int j = 0; j < 3; j++ )
...
Brush::~Brush() {
	// free faces
	face_t *next;
	for( face_t *f = brush_faces; f; f = next ) {
		next = f->next;
		Face_Free( f );
	}
...
void Brush::SetTexture( texdef_t *texdef ) {
	for( face_t *f = brush_faces; f; f = f->next ) {
		f->texdef = *texdef;
	}
...
==============
*/
void Brush::SelectFaceForDragging( face_t *f, bool shear ) {
	int		i;
	face_t *f2;
	winding_t *w;
	float	d;
...
void Brush::BuildWindings() {
	...
	for( ; face; face = face->next ) {
		...
		BeginTexturingFace( this, face, face->d_texture );
...
void Brush::Draw( const huang::Viewport *viewport ) {
	face_t *face;
	int     order;
	winding_t *w;
	int        i;
	...
	for( face = brush_faces, order = 0; face; face = face->next, order++ ) {
		...
		if( viewport->GetDrawMode() == huang::DRAW_WIREFRAME ) {
			// draw the polygon
			...
			if( face->d_texture != prev && viewport->GetDrawMode() == huang::DRAW_TEXTURED ) {
			...
			glBegin( GL_POLYGON );
			for( i = 0; i < w->numpoints; i++ ) {
				if( viewport->GetDrawMode() == huang::DRAW_TEXTURED ) {

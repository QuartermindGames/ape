	...
	if( !( flags & SF_SELECTED_ONLY ) )
		for( brush = active_brushes.next; brush != &active_brushes; brush = brush->next ) {
			...
			if( FilterBrush( brush ) )
				continue;
			face = brush->Ray( origin, dir, &dist );
...
void Select_Brush( Brush *brush ) {
	...
	if( e ) {
		...
		singleselect:
			...
			brush->AddToList( &selected_brushes );
		}
...
void Select_Ray( vec3_t origin, vec3_t dir, int flags ) {
	...
	if( t.selected ) {
		...
		t.brush->AddToList( &active_brushes );
	} else {

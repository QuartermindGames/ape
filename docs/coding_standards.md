# OSW Coding Standards

This engine and libraries are primarily in C, with some external components in C++. 
These rules apply to everything except for any third-party libraries, which will have their own code style.

Fundamentally, the style to be followed is the same as GTK, which can be found [here](https://developer.gnome.org/documentation/guidelines/programming/coding-style.html).

- For private members, we should postfix with underscore rather than prefix.

...
under `<project>/textures/` instead, which is unfortunately due to a tooling limitation. That said, whenever possible,
the former is preferable.

APE Tech has support for a large range of different formats, that said we would recommend using industry standard
